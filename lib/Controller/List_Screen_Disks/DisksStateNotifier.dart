...
class DisksStateNotifier extends StateNotifier<DisksStates> {
  DisksStateNotifier() : super(DisksLoadingState());

  fetchDisk() async {
    try {
      var querry =
          await FirebaseFirestore.instance.collection('list_disk_data').get();
      final document = querry.docs;
      List<Chair> list = [];
      for (var x in document) {
        list.add(Chair.fromMap(x.data()));
      }
      state = DisksLoadedState(list: list);
    } catch (e) {
